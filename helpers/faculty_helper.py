...
def get_grade_distribution_by_faculty(db, teacher_name: str, semester_id: int, subject_code: str = None):
    ...
    Returns a pandas DataFrame with grade percentages and totals.
    """
    ...
    if not data:
        print("[INFO] No grade distribution data found.")
        return pd.DataFrame()

    records = []
    for item in data:
        _id = item.get("_id", {})
        program_code = _id.get("programCode", "N/A")
        program_name = _id.get("programName", "Unknown Program")

        grades = pd.to_numeric(pd.Series(item.get("grades", [])), errors="coerce").dropna().astype(int)
        ...
        for key, value in bins.items():
            record[key] = f"{(value / total_grades) * 100:.2f}%"
    ...
    for col in column_order:
        if col not in df:
            df[col] = "0.00%" if col != "Total" else 0
